...
fn synthetic_facts() -> TokenizerFacts {
    let tokens: Vec<String> = (0..EXPECTED_VOCAB_SIZE)
        .map(|i| format!("tok{i}"))
        .collect();
...
fn probe_p10_lazy_dog() {
    let tok = pinned_tokenizer();
    assert_eq!(
        tok.encode("The quick brown fox jumps over the lazy dog", true)
            .unwrap(),
...
fn probe_p11_leading_spaces() {
    ...
    let tok = pinned_tokenizer();
    assert_eq!(
        tok.encode("  leading spaces", true).unwrap(),
        vec![216, 2899, 5600]
    );
}
...
";
    ...
    let expected: Vec<u32> = vec![
        504, 7080, 1088, 25316, 12927, 18411, 259, 2257, 284, 24208, 357, 738, 260, 10560, 28, 260,
        23864, 2318, 28, 284, 260, 2909, 12914, 618, 260, 452, 55, 69, 54, 29709, 4624, 30, 378,
        29709, 45438, 3324, 1005, 8844, 3082, 2859, 335, 10910, 32688, 1015, 260, 24672, 25817, 30,
        24672, 37736, 1806, 876, 327, 6736, 4261, 284, 327, 260, 30167, 2674, 11498, 30, 378, 1743,
        10379, 2951, 281, 260, 1606, 36, 79, 59, 79, 61, 3608, 10220, 28, 527, 260, 24672, 7555,
        366, 26420, 418, 3509, 655, 30, 5883, 2095, 7313, 738, 260, 450, 4130, 81, 6064, 980, 28,
        527, 28263, 354, 17108, 11423, 22223, 284, 253, 10193, 41880, 12793, 2376, 30, 378, 6011,
        5239, 284, 260, 26420, 5239, 359, 6090, 13624, 588, 260, 32045, 34349, 1230, 2093, 28602,
        3509, 655, 355, 4478, 388, 351, 4686, 30, 330, 1890, 2468, 29278, 6379, 260, 4478, 388,
        2050, 351, 253, 15665, 1230, 282, 17837, 28, 979, 253, 2955, 29278, 6379, 253, 2073, 7717,
        282, 2674, 8648, 690, 253, 8510, 282, 1694, 30, 5081, 26211, 1658, 314, 11834, 1523, 260,
        8268, 3924, 4207, 1508, 14204, 588, 338, 1844, 359, 41088, 28, 284, 897, 29278, 2301, 314,
        15345, 29, 9955, 3924, 1092, 750, 7585, 314, 2473, 30,
    ];
    assert_eq!(ids, expected);
}
...
";
    ...
    let expected: Vec<u32> = vec![
        54, 369, 259, 314, 253, 1734, 1789, 327, 2194, 5646, 3253, 30, 378, 25316, 13281, 314,
        7074, 618, 4073, 5933, 42, 19839, 274, 28, 34868, 28, 1462, 6558, 28, 1502, 11160, 28,
        16067, 288, 372, 5810, 28, 284, 5087, 2909, 4686, 30, 3768, 3632, 13662, 335, 354, 44771,
        14019, 6133, 284, 4631, 28713, 351, 8212, 2257, 6332, 30, 1068, 23630, 746, 12936, 28,
        18411, 259, 2774, 946, 618, 260, 23630, 29709, 28, 527, 13101, 3324, 1005, 1743, 4577, 284,
        27795, 2476, 13104, 4261, 288, 260, 768, 5181, 25817, 1770, 30, 1985, 10910, 32688, 260,
        24672, 25817, 314, 8996, 975, 357, 8211, 1507, 18771, 284, 30167, 2674, 32528, 7878, 281,
        19775, 3500, 30, 378, 29709, 28263, 253, 1165, 28, 30473, 12077, 2376, 588, 338, 3279,
        2909, 1072, 441, 3749, 335, 750, 1542, 25817, 6230, 30, 14980, 1005, 10379, 1066, 260, 452,
        55, 69, 54, 3608, 10220, 28, 284, 260, 29709, 988, 3237, 971, 3608, 4624, 981, 260, 6736,
        17109, 30, 669, 1157, 8211, 260, 2609, 2050, 1890, 42, 253, 6011, 314, 10717, 618, 17837,
        28, 260, 17837, 359, 4180, 288, 260, 1743, 28, 284, 260, 23146, 3124, 314, 2198, 2520,
        1056, 288, 260, 38448, 30, 1943, 282, 260, 14019, 3301, 3073, 23124, 940, 1699, 980, 6177,
        28, 975, 260, 2444, 13281, 7313, 281, 582, 980, 351, 253, 3600, 2468, 30, 669, 8510, 314,
        18519, 9174, 588, 338, 6514, 722, 281, 253, 986, 2468, 29278, 1072, 441, 8542, 260, 1743,
        1731, 750, 1542, 4234, 30, 657, 5961, 805, 288, 1538, 253, 43524, 1902, 282, 2468, 1694,
        327, 260, 26211, 28, 284, 357, 314, 16551, 429, 7094, 39799, 15590, 338, 15121, 3869, 30,
        198, 198, 54, 369, 259, 314, 253, 1734, 1789, 327, 2194, 5646, 3253, 30, 378, 25316, 13281,
        314, 7074, 618, 4073, 5933, 42, 19839, 274, 28, 34868, 28, 1462, 6558, 28, 1502, 11160, 28,
        16067, 288, 372, 5810, 28, 284, 5087, 2909, 4686, 30, 3768, 3632, 13662, 335, 354, 44771,
        14019, 6133, 284, 4631, 28713, 351, 8212, 2257, 6332, 30, 1068, 23630, 746, 12936, 28,
        18411, 259, 2774, 946, 618, 260, 23630, 29709, 28, 527, 13101, 3324, 1005, 1743, 4577, 284,
        27795, 2476, 13104, 4261, 288, 260, 768, 5181, 25817, 1770, 30, 1985, 10910, 32688, 260,
...
fn behavior_specials_cache() {
    let tok = pinned_tokenizer();
    let specials: Vec<(String, u32)> = tok
        .special_tokens()
...
fn negative_wrong_model() {
    ...
    assert!(matches!(
        Gpt2BpeTokenizer::new(f),
        Err(TokenizerError::UnsupportedModel { .. })
    ));
}
...
fn negative_wrong_pre() {
    ...
    assert!(matches!(
        Gpt2BpeTokenizer::new(f),
        Err(TokenizerError::UnsupportedPreTokenizer { .. })
    ));
}
...
fn negative_vocab_count() {
    ...
    assert!(matches!(
        Gpt2BpeTokenizer::new(f),
        Err(TokenizerError::VocabSizeMismatch { .. })
    ));
}
...
fn negative_merges_count() {
    ...
    assert!(matches!(
        Gpt2BpeTokenizer::new(f),
        Err(TokenizerError::MergesCountMismatch { .. })
    ));
}
...
fn negative_token_type_count() {
    ...
    assert!(matches!(
        Gpt2BpeTokenizer::new(f),
        Err(TokenizerError::TokenTypeCountMismatch { .. })
    ));
}
...
fn negative_scores_present() {
    ...
    assert!(matches!(
        Gpt2BpeTokenizer::new(f),
        Err(TokenizerError::ScoresPresent)
    ));
}
...
fn negative_malformed_merge() {
    ...
    assert!(matches!(
        Gpt2BpeTokenizer::new(f),
        Err(TokenizerError::MalformedMergeString { .. })
    ));
}
...
fn negative_byte_boundary_space_token() {
    ...
    assert!(matches!(
        Gpt2BpeTokenizer::new(f),
        Err(TokenizerError::ByteBoundaryViolation { .. })
    ));
}
...
fn negative_token_id_out_of_range() {
    ...
    assert!(matches!(
        Gpt2BpeTokenizer::new(f),
        Err(TokenizerError::TokenIdOutOfRange { .. })
    ));
}
...
fn negative_add_bos_true() {
    ...
    assert!(matches!(
        Gpt2BpeTokenizer::new(f),
        Err(TokenizerError::AddBosTokenNotSupported { .. })
    ));
}
...
fn negative_add_space_prefix_true() {
    ...
    assert!(matches!(
        Gpt2BpeTokenizer::new(f),
        Err(TokenizerError::AddSpacePrefixNotSupported { .. })
    ));
}
