...
//!   its facts — `GgmlType`, `ByteRange`).
//! - The GI2-1 `purpose=cpu-oracle` dequant (`dequant.rs`) is explicitly NOT
//!   a backend representation; the declared f32 conversion **reuses its
//!   exact dequant semantics** (`ggml/src/ggml-quants.c @ a957b7747`,
//!   [`crate::dequant::ORACLE_TRANSFORM_IMPL`]) as the initial admitted
...
use crate::gguf::{GgmlType, PINNED_SHA256_HEX};
...
pub struct RepackDescriptor {
    ...
    /// The pinned transform the conversion reuses (`dequant.rs` semantics:
    /// `ggml/src/ggml-quants.c @ a957b7747`).
...
impl RepackSelection {
    ...
    #[must_use]
    pub fn f32_conversion_descriptor(&self, class: GgmlType) -> Option<&RepackDescriptor> {
    ...
    #[must_use]
    pub fn class(&self, class: GgmlType) -> Option<&PerClassSelection> {
        self.per_class
            .iter()
    ...
    #[must_use]
    pub fn representation(&self, class: GgmlType) -> Option<&SelectedRepresentation> {
