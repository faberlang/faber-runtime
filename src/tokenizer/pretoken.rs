//! `smollm` pre-tokenizer stage for the pinned gpt2 row.
//!
//! Mirrors llama.cpp 10150 `unicode_regex_split` + `unicode_regex_split_custom_gpt2`
//! (reference details in the parent module docs). Two sequential regex passes
//! over codepoints:
//!
//! 1. `\p{N}` — each number codepoint becomes its own fragment.
//! 2. The GPT2 custom splitter `'s|'t|'re|'ve|'m|'ll|'d| ?\p{L}+| ?\p{N}+|
//!    ?[^\s\p{L}\p{N}]+|\s+(?!\S)`, implemented directly over the codepoint
//!    flag table below (identical to `unicode_regex_split_custom_gpt2`).
//!
//! `byte_encode` then maps every byte of every fragment through the GPT2
//! byte-to-display table (space → `Ġ` U+0120, newline → `Ċ` U+010A, …), the
//! display form the BPE stage merges on.
//!
//! The embedded tables (`CPT_RANGES`, `WHITESPACE_SET`, `BYTE_TO_DISPLAY`) are
//! the exact data the pinned comparator executes (see provenance in the parent
//! module docs) and are private to this module.

/// `unicode_regex_split` for the `smollm` pre pair — returns byte spans.
pub(super) fn pre_tokenize_smollm(text: &str) -> Vec<(usize, usize)> {
...
pub(super) fn byte_encode(word: &str) -> String {
...
const BYTE_TO_DISPLAY: [char; 256] = [
