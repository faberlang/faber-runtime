...
| Shape and indexing | Rank, shape, element count, construction from flat data, reshape, flat offset calculation, get, set, and fill are implemented with explicit negative-dimension, negative-index, out-of-bounds, mismatch, and overflow checks. The LLVM host tensor ABI admits only `F32`, `F64`, `I32`, and `I64`, matching its arithmetic/reduction dispatch set; narrower scalar families such as `F16`, `I16`, and unsigned integers fail at tensor construction/import rather than later arithmetic. | `src/tensor.rs`; `src/tensor_test.rs`; `hosts/llvm/src/tensor.rs`; `hosts/llvm/src/lib_test.rs::tensor_host_rejects_kinds_without_arithmetic_dispatch_at_admission` |
| Elementwise arithmetic | Dense tensors support broadcast-compatible add, subtract, multiply, checked finite f32 division, f32 negation, and f32 scalar scaling. Broadcast mismatches fail closed with `ERR_BROADCAST_SHAPE`. Division rejects non-finite inputs, exact zero denominators, and non-finite results before any autograd VJP claim. | `Tensor::addita`, `Tensor::subtrahe`, `Tensor::multiplica`, `Tensor<f32>::divide`, `Tensor<f32>::reciproca`, `Tensor<f32>::neg`, `Tensor<f32>::scala`; `src/tensor_test.rs::addita_broadcasts_size_one_dimension`; `src/tensor_test.rs::divide_broadcasts_finite_f32_tensors`; `src/tensor_test.rs::reciproca_preserves_shape_and_checks_denominators`; `src/tensor_test.rs::neg_negates_f32_elements_and_preserves_shape`; `src/tensor_test.rs::scala_scales_f32_elements_and_preserves_shape` |
...
| Reductions | The Rust carrier exposes `summa` as an element-type sum and `Tensor<f32>::media` as a non-empty f32 mean. The LLVM host ABI also exposes `__faber_rt_v1_tensor_sum` for `F32`, `F64`, `I32`, and `I64`, and float-only `__faber_rt_v1_tensor_mean`; integer mean is rejected until conversion support is honest for that path. | `src/tensor.rs`; `src/tensor_test.rs::media_averages_f32_elements_and_rejects_empty_tensor`; `hosts/llvm/src/tensor.rs`; `hosts/llvm/src/lib_test.rs::tensor_host_arithmetic_boundary_supports_f32_f64_i32_i64` |
...
| Autograd scaffold | The Rust runtime has an internal dense `Tensor<f32>` tape with node ids, parent edges, saved forward tensors, exact-shape gradient accumulation, duplicate-parent accumulation, scalar-loss backward, broadcast reductions for add/sub/mul/divide, rank-2 matmul VJP, tape-owned `neg`, tape-owned scalar `scala`, tape-owned `forma` reshape gradients, tape-owned materialized `permute` with inverse-permutation backward, broadcast-aware backward after `permute` for scalar/vector/size-one tensor operands, tape-owned axis-0 `sectio` with parent-gradient scatter-add, tape-owned `media` mean backward, tape-owned `crux_entropia` cross-entropy loss VJP, and fail-closed leaf rejection for raw aliased `sectio` views. Materialized `sectio` copies are accepted and snapshotted like other leaves. | `src/autograd.rs`; `src/autograd.rs::tests::gradient_accumulation_rejects_broadcast_compatible_wrong_shape`; `src/autograd.rs::tests::backward_matches_rank2_matmul_sum_vjp`; `src/autograd.rs::tests::backward_divides_same_shape_gradients`; `src/autograd.rs::tests::backward_reduces_broadcast_divide_denominator_gradient`; `src/autograd.rs::tests::backward_negates_tape_owned_neg_gradient`; `src/autograd.rs::tests::backward_scales_tape_owned_scala_gradient_by_factor`; `src/autograd.rs::tests::backward_zero_scala_branch_does_not_mask_repeated_parent_gradient`; `src/autograd.rs::tests::backward_reshapes_tape_owned_forma_gradient_into_parent_shape`; `src/autograd.rs::tests::backward_inverts_tape_owned_permute_gradient_into_parent_shape`; `src/autograd.rs::tests::backward_reduces_scalar_broadcast_after_permute`; `src/autograd.rs::tests::backward_reduces_vector_broadcast_mul_after_permute`; `src/autograd.rs::tests::backward_reduces_tensor_broadcast_sub_after_permute`; `src/autograd.rs::tests::backward_scatter_adds_tape_owned_sectio_gradient_into_parent`; `src/autograd.rs::tests::backward_distributes_tape_owned_media_gradient_over_parent`; `src/autograd.rs::tests::autograd_rejects_raw_sectio_view_leaf`; `src/autograd.rs::tests::autograd_materialized_sectio_snapshot_ignores_parent_alias_mutation` |
| Finite-difference oracle | Test-only central-difference checks cover rank-0 scalar `x * x + x`, the exact rung-3 scalar target `loss(x, weight, target) = (x * weight - target)^2` with `x=2.0`, `weight=3.0`, `target=4.0`, `loss=4.0`, and `d_weight ~= 8.0`, same-shape vector, broadcast-bias, broadcast-scale, mean-square, scalar-scaled mean-square, finite division-square, and neg-linear losses, plus a dense linear training-step mean loss `media((XW + b - target)^2)` that compares autograd gradients for input, weight, and bias against CPU finite differences. A test-only `TestOnlySgdSession` oracle owns a flat parameter set with frozen input slots and trainable weight/bias slots, computes fresh per-step gradients, zeros frozen input gradients before update, applies manual `param -= learning_rate * grad` updates, compares updated parameters against the finite-difference session step, and checks a two-step loss trace matches the finite-difference trace while strictly decreasing. | `src/autograd_reference_test.rs` |
| Gelu activation | `Tensor<f32>::gelu()` with finite-input validation; `AutogradOp::Gelu` with analytical VJP using inline `f32::tanh()`; finite-difference oracle verified at `test_autograd_gelu_gradient`. No AIR generated-backward — tanh not in generated-backward pipeline. | `src/tensor.rs`; `src/autograd.rs`; `src/autograd_reference_test.rs::test_autograd_gelu_gradient` |
| Softmax forward | `Tensor<f32>::softmax()` with numerical stability transform (x - max(x)), rank-1 and rank-2 row-wise. VJP integrated via cross-entropy loss (`crux_entropia`). | `src/tensor.rs` |
| Cross-entropy loss | `Tensor<f32>::crux_entropia` with analytical VJP `(softmax - targets) / N`; FD oracle verified. | `src/tensor.rs`; `src/autograd.rs`; `src/autograd_reference_test.rs` |
...
- `Tensor<f32>::neg` and tape-owned `AutogradTape::neg` provide the first true
  ...
  LayerNorm (analytical VJP per Ba et al. 2016) is implemented with oracle
  verification at `src/autograd_reference_test.rs::test_autograd_layernorm_gradient`.
  Combined with ReLU and Sqrt, the Activations/layernorm/softmax/attention coverage
  moves from approximately 20% to 35%.
  Gelu implemented with VJP and finite-difference oracle; Cross-entropy loss VJP integrated with `crux_entropia`; Exp, Log, sin, cos, tanh remain pending.
...
| 2 | Numeric unary primitives (activation v1) | `Tensor<f32>::neg` and `AutogradTape::neg` prove shape-preserving unary linearity with VJP `-upstream`. `Tensor::forma` remains the reshape proof. ReLU (`max(0, x)`) and Sqrt (`sqrt(x)`) are now implemented with domain policy from `radix/docs/design/domain-primitive-policy.md`, autograd VJPs, and finite-difference coverage. Gelu implemented with VJP; Softmax forward-only; Exp, Log, sin, cos, tanh remain pending (deferred). | Pick any next domain-sensitive primitive only after non-finite/domain behavior is specified; prove tensor forward plus tape VJP against finite differences. | No math-library surface, activation library, or PyTorch unary parity. |
...
| 5 | LayerNorm (analytical VJP) | `Tensor<f32>::layernorm` with domain validation; `AutogradOp::LayerNorm` with Ba et al. 2016 VJP; finite-difference oracle verified. | Keep runtime-local unless promoted to host ABI or generated-gradient; no device-native kernel. | No AIR/compiler-owned gradient, host ABI gradient handle, or device-native kernel. |
...
2. Restrict the proof graph to materialized dense elementwise add/sub/mul,
   broadcast add/sub/mul/divide, rank-2 matmul, tape-owned `neg`, tape-owned
   scalar `scala`, tape-owned `forma`, materialized tape-owned `permute`,
   tape-owned axis-0 `sectio`, `summa`, and non-empty f32
...
ABI design. Domain-sensitive numeric unary primitives such as exp/log/sin can
follow once their Tensor-level operations and policies exist.
